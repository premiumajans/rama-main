...
const ServiceItem = ({portfolio}: { portfolio: productItem }) => {
    const [slideIndex, setSlideIndex] = useState(0);
    const [toggler, setToggler] = useState(false);
    const {query} = useRouter()
    const {i18n} = useTranslation('common')
    const {photos, photo, translations, created_at} = portfolio
...
export async function getServerSideProps(context: any) {
    const {query} = context
    const portfolio = await fetch(process.env['NEXT_PUBLIC_MAIN_PATH'] + '/portfolio/' + query.portfolioId)
