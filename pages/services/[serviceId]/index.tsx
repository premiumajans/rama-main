...
const ServiceItem = ({service}: { service: productItem }) => {
    const {query} = useRouter()
    const [slideIndex, setSlideIndex] = useState(0);
    const [toggler, setToggler] = useState(false);
    const {i18n} = useTranslation('common')
    const {photos, photo, translations, created_at} = service
    ...
    return <>
        <Head>
            ...
            <meta property="og:url"
                  content={process.env.NEXT_PUBLIC_MAIN_PATH_WITHOUT_API + 'writer/' + query.serviceId}/>
        </Head>
...
export async function getServerSideProps(context: any) {
    const {query} = context
    const service = await fetch(process.env['NEXT_PUBLIC_MAIN_PATH'] + '/service/' + query.serviceId)
