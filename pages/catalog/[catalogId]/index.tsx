...
const CatalogItem = ({catalog}: { catalog: productItem }) => {
    const [slideIndex, setSlideIndex] = useState(0);
    const [toggler, setToggler] = useState(false);
    const {query} = useRouter()
    const {i18n} = useTranslation('common')
    const {photos, photo, translations, created_at} = catalog
                                ...
                                <div className="section-container p-l-r">
                                    <div className="post-details no-sidebar">
                                        ...
                                        <div className="post-meta">
                                            ...
                                            <span className=""><i
                                                className="icon_clock_alt"></i>{transformDate(created_at)}</span>
                                        </div>
...
export async function getServerSideProps(context: any) {
    const {query} = context
    const catalog = await fetch(process.env['NEXT_PUBLIC_MAIN_PATH'] + '/catalog/' + query.catalogId)
